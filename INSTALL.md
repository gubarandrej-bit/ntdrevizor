...
Окно контекста локальной модели — `LOCAL_GGUF_N_CTX` в `.env` (по умолчанию 4096). Если в отчёте встречается «Requested tokens … exceed context window» — модель получила слишком длинный промпт: система теперь сама обрезает промпт под окно (с пометкой), но при желании можно поднять окно до 8192 (для 1,5B-модели это безопасно, расход ОЗУ растёт незначительно).
...
Сканированные PDF без текстового слоя идут в Tesseract (`tesseract-ocr-rus`). OCR работает **постранично**: в смешанном PDF векторные листы разбираются обычным путём, а растровые страницы без текста распознаются отдельно (PyMuPDF-растеризация + tesseract). Текст OCR помечается как ненадёжный. Качество OCR на старом CPU низкое; при пустом результате проверка схем не проводится, ничего не дорисовывается.
