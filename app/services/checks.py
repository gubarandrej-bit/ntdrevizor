...
def check_equipment_compat(spec_items: list[dict], text: str = "") -> dict[str, Any]:
    ...
    # 6а) устройства с нераспознанной линейкой — совместимость не подтверждена.
    # Позиции, похожие на приборы/блоки/извещатели и т.п., чья марка не входит
    # ни в одну линейку каталога, перечисляются явно: молча «совместимо» для них
    # не предполагается.
    _DEVICE_WORDS = (
        "прибор", "блок", "модул", "извещател", "оповещател", "контроллер",
        "расширител", "источник", "панел", "сервер", "коммутатор", "шкаф",
        "устройств", "датчик", "монитор", "регистратор", "камер", "считывател",
        "пульт", "клавиатур", "преобразовател", "громкоговорител", "табло",
    )
    recognized_ids = {
        id(i)
        for i in eq
        if any(any(t in _eq_blob(i) for t in e.get("tokens") or []) for e in eco_list)
    }
    unknown_devices = [
        i for i in eq
        if id(i) not in recognized_ids and any(w in _eq_blob(i) for w in _DEVICE_WORDS)
    ]
    if unknown_devices:
        marks = _marks_of(unknown_devices, 12)
        findings.append(
            finding(
                "info",
                "Совместимость части устройств не подтверждена автоматически",
                "Устройства, линейка/производитель которых не в каталоге (протокольная "
                "совместимость не проверялась — добавьте марки в каталог equipment_compat "
                "или проверьте вручную): "
                + ", ".join(marks)
                + ("…" if len(marks) >= 12 else "")
                + ".",
                ["ГОСТ Р 53325-2012"],
                evidence=f"устройств без распознанной линейки: {len(unknown_devices)}",
            )
        )
